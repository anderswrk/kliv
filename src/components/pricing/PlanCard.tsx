...
export function PlanCard({ plan, plans, buttonMessage, onButtonClick }: PlanCardProps) {
  ...
  const getLocalizedFeatures = (productName: string, originalFeatures: string) => {
    if (!productName) return originalFeatures;
    const key = `planContent.features.${productName}`;
    const translated = t(key);
  ...
  const localizedCurrentPlan = {
    ...
    features: getLocalizedFeatures(currentPlan.productName, currentPlan.features),
  ...
  const localizedPlans = plans ? plans.map(plan => ({
    ...
    features: getLocalizedFeatures(plan.productName, plan.features),
