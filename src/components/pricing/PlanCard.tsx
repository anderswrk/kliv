...
export function PlanCard({plan, plans, buttonMessage, onButtonClick}: PlanCardProps) {
    ...
    const getLocalizedFeatures = (groupName: string | undefined, productName: string, originalFeatures: string) => {
        // Try productName first, then group name
        for (const name of [productName, groupName]) {
            if (!name) continue;
            const key = `planContent.features.${name}`;
            const translated = t(key);
    ...
    const getLocalizedDescription = (groupName: string | undefined, productName: string) => {
        ...
        for (const name of [productName, groupName]) {
            if (!name) continue;
            const key = `planContent.descriptions.${name}`;
            const translated = t(key);
            if (translated !== key) return translated;
        }
        return '';
        ...
        <div className="plan-card">
            {isHighlighted ? (
                ...
                <div className={`plan-card-normal group ${
                    (currentPlan.group === 'Pro' || currentPlan.productName === 'Pro') 
                        ? 'preferred' 
                        : ''
                }`}>
                    <div className="plan-card-body">
...
function PlanCardContent({
    ...
    return (
        <>
            ...
            {/* Title */}
            <div className="title">
                {currentPlan.productName || currentPlan.group || currentPlan.name}
            </div>
