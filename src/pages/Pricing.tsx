...
export function Pricing() {
    ...
    // Separate recurring plans from URL plans (Custom/Enterprise) and sort by price
    const recurringPlans = filteredPlans
        .filter(plan => plan.planType !== 'url')
        .sort((a, b) => (a.unitAmount || 0) - (b.unitAmount || 0));
    const urlPlans = filteredPlans.filter(plan => plan.planType === 'url');
                    ...
                    {pricingData && !loading && !error && (
                        <div className="max-w-7xl mx-auto">
                            <div className="plan-container">
                                ...
                                {/* Show each paid plan individually */}
                                ...
                                {/* Show URL-based plans (Custom/Enterprise) */}
                                {urlPlans.map((plan) => (
                                    <PlanCard
                                        key={plan.uuid}
                                        plan={plan}
                                        buttonMessage={t('pricing.contactUs')}
                                        onButtonClick={handlePaidPlanClick}
                                    />
                                ))}
                    ...
                    <div className="max-w-6xl mx-auto">
                        ...
                        <div className="overflow-x-auto">
                            <table className="w-full border-collapse">
                                    ...
                                    <tr className="border-b-2 border-border">
                                        ...
                                        <th className="text-center p-3">
                                            ...
                                            <button
                                                ...
                                                className="px-3 py-2 rounded-lg bg-primary text-primary-foreground hover:bg-primary/90 transition-colors text-sm font-semibold w-full max-w-[130px]"
                                            >
                                                {t('pricing.getStarted')}
                                            </button>
                                        </th>
                                        <th className="text-center p-3">
                                            <div className="font-bold text-foreground mb-3">{t('planContent.plans.Pro')}</div>
                                            <button
                                                onClick={() => recurringPlans.length > 0 && handlePaidPlanClick(recurringPlans[0])}
                                                className="px-3 py-2 rounded-lg bg-orange-500 text-white hover:bg-orange-600 transition-colors text-sm font-semibold w-full max-w-[130px] shadow-md"
                                            >
                                                {t('pricing.subscribe')}
                                            </button>
                                        </th>
                                        <th className="text-center p-3">
                                            <div className="font-bold text-foreground mb-3">{t('planContent.plans.Max 5x')}</div>
                                            <button
                                                onClick={() => recurringPlans.length > 1 && handlePaidPlanClick(recurringPlans[1])}
                                                className="px-3 py-2 rounded-lg bg-primary text-primary-foreground hover:bg-primary/90 transition-colors text-sm font-semibold w-full max-w-[130px]"
                                            >
                                                {t('pricing.subscribe')}
                                            </button>
                                        ...
                                        <th className="text-center p-3">
                                            <div className="font-bold text-foreground mb-3">{t('planContent.plans.Max 20x')}</div>
                                            <button
                                                onClick={() => recurringPlans.length > 2 && handlePaidPlanClick(recurringPlans[2])}
                                                className="px-3 py-2 rounded-lg bg-primary text-primary-foreground hover:bg-primary/90 transition-colors text-sm font-semibold w-full max-w-[130px]"
                                            >
                                                {t('pricing.subscribe')}
                                            </button>
                                        ...
                                        <th className="text-center p-3">
                                            ...
                                            <LocalizedLink 
                                                to="/contact"
                                                className="px-3 py-2 rounded-lg bg-secondary text-secondary-foreground hover:bg-secondary/80 transition-colors text-sm font-semibold w-full max-w-[130px] block text-center"
                                            >
                                                {t('pricing.contactUs')}
                                            </LocalizedLink>
                                        </th>
                                ...
                                <tbody>
                                    {(['publicProjects', 'hosting', 'database', 'aiCredits', 'support', 'privateProjects', 'customDomains', 'branding', 'collaboration', 'analytics', 'sso', 'enclave'] as const).map((feature) => (
                                        <tr key={feature} className="border-b border-border/50 hover:bg-muted/30 transition-colors">
                                            <td className="p-4 font-medium text-foreground">{t(`planContent.featureMatrix.features.${feature}`)}</td>
                                            {(['free', 'pro', 'max5x', 'max20x', 'custom'] as const).map((plan) => (
                                                <td key={plan} className="p-3 text-center text-muted-foreground text-sm">
                                                    {renderFeatureValue(t(`planContent.featureMatrix.values.${plan}.${feature}`))}
                                                </td>
                                            ))}
                                        </tr>
                                    ))}
